...
export function QuestionCard(props: {
  ...
  return (
    <section className={`question-card ${props.feedback}`}>
      ...
      <form onSubmit={e => { e.preventDefault(); props.onSubmit(); }}>
        <input
          ref={ref} autoFocus disabled={props.disabled}
          value={props.value} inputMode="numeric"
          placeholder="Type answer + Enter"
          onChange={e => props.onChange(clean(e.target.value))}
        />
      </form>
      <div className="feedback">
        {props.feedback === 'correct' ? 'Correct!' : props.feedback === 'wrong' ? `Wrong — answer: ${props.correctAnswer}` : ''}
      </div>
    </section>
  );
}
